...
resource "aws_appautoscaling_target" "autoscaling_target" {
  ...
  role_arn           = local.ecs_autoscale_role_arn
  min_capacity       = var.min_capacity
  max_capacity       = var.max_capacity
}

# define upscalling Policy
resource "aws_appautoscaling_policy" "upscalling_policy" {
  name               = "${var.name_prefix}-upscaling-policy"
  service_namespace  = "ecs"
  resource_id        = "service/${aws_ecs_cluster.ecs_cluster.name}/${aws_ecs_service.ecs_service.name}"
  scalable_dimension = "ecs:service:DesiredCount"
...
resource "aws_cloudwatch_metric_alarm" "upscaling_metric_alarm" {
  alarm_name          = "${var.name_prefix}-upscalling-metric-alarm"
  ...
  alarm_actions = ["${aws_appautoscaling_policy.upscalling_policy.arn}"]
}
