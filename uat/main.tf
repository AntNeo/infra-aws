...
module "ecs" {
  source              = "..//modules/ecs"
  app_vpc_id          = module.uatvpc.vpc_id
  vpc_private_subnets = module.uatvpc.private_subnets
...
resource "aws_vpc_peering_connection" "foo" {
  ...
  tags = {
    Name = "VPC Peering between infra and uat"
  }
}
...
resource "aws_route" "alb_to_ecs" {
  for_each                  = toset(module.uatvpc.private_subnets_rts)
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect = "Allow"
        Resource = [
          "${module.uat-frontend.bucket_arn}",
          "${module.uat-frontend.bucket_arn}/*"
        ]
      }
    ]
